...
let inputNumber = value => {
    if ((disp.slice(disp.length - 1, disp.length) !== ')')) {
        if (equalFl == 1) { //演算後の入力をする時
            inputClear();
            equalFl = 0;
        }
        if (value == '.') { //小数点が入力されたとき
            if (firstFl == 0 && minusFl == 0) { //その項の最初の入力が小数点だった時
                disp += '0.'; //0.○○
            ...
            else if (dotFl != 1 && minusFl == 0) { //その項で小数点がまだ無いとき
                disp += '.'; //1~9.○○
                document.getElementById('d_box').textContent = disp;
                console.log('.');
                numFl = 0;
        ...
        } else if ((firstFl == 1 || (firstFl == 0 && value != '0' && value != '00')) && minusFl == 0) { //数値が入力された時
            disp += value;
            document.getElementById('d_box').textContent = disp;
            console.log(value);
            numFl = 1;
...
//+/-が入力された時
let inputSign = () => {
    if (equalFl == 0 && String(result).length == 1)
        changeSign();
    else if (equalFl == 0 && String(result).slice(String(result).length - 1, String(result).length) !== '.')
        changeSign();
}
let changeSign = () => {
    if (result > 0) { //数値が正の時
